...
class DownRepConv_Block_v2(nn.Module):
    def __init__(self, module_nums, channel_nums, act_type, scale, colors):
        ...
        self.input_conv2 = RepBlockV2(inp_planes=self.colors, out_planes=self.colors*self.scale*self.scale, act_type=self.act_type)
...
class DownRepConv_Block_v2_deploy(nn.Module):
    def __init__(self, module_nums, channel_nums, act_type, scale, colors):
        ...
        self.upsampler = nn.PixelShuffle(3)
        self.input_conv2 = Conv3X3(inp_planes=self.colors, out_planes=self.colors*self.scale*self.scale, act_type=self.act_type)
